...
export default function OneItem() {
  ...
  return (<>
    <div className="card1">
      ...
      <div>
        <div>
          <img src={res.image===null?defaultImagePath:res.links.mission_patch} alt="spaceX"/>
        </div>
        <h2>{res.title}</h2>
        <p>{typeof res.details==="string"?res.details:"" }</p>
        <div className="dateandsite"><span>{res.launch_date_local.slice(0 , 10)}</span><span>{res.launch_site.site_id}</span></div>
